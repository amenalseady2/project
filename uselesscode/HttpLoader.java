/*
package com.shanggame.news.net;
...
import com.shanggame.news.NewsApplication;
import com.shanggame.news.listener.JsonArrayListener;
import com.shanggame.news.listener.JsonObjectListener;
import com.shanggame.news.utils.LogManager;
import com.shanggame.news.utils.ToastUtil;
...
/**
    ...
    private static RequestQueue mRequestQueue = Volley.newRequestQueue(NewsApplication.getInstance());
...
/**
    ...
    public static void post(String url, Map<String, String> params,final JsonArrayListener listener) {
        request(Request.Method.POST, url, params, requestCode, new ResponseListener() {

            @Override
            public void onGetResponseSuccess(int requestCode, String response) {
                getResult(response,listener);
            }

            @Override
            public void onGetResponseError(int requestCode, VolleyError error) {
                LogManager.i(error.getNetworkTimeMs()+"");
                LogManager.i(error.getMessage());
                LogManager.i(error.networkResponse.toString());
...
/**
    ...
    public static void post(String url, Map<String, String> params,final JsonObjectListener listener) {
        request(Request.Method.POST, url, params, requestCode, new ResponseListener() {
            @Override
            public void onGetResponseSuccess(int requestCode, String response) {
                getResult(response,listener);
...
/**
    ...
    private static GsonRequest makeRBRequest(int method, String url, Map<String, String> params, int requestCode, ResponseListener listener, boolean isCache) {
        ...
        GsonRequest request = new GsonRequest<String>(method, url, params, requestListener, requestListener, isCache) {
            @Override
...
/**
    ...
    private static class RequestListener implements Response.ErrorListener, Response.Listener<String> {
...
//            try {
//                //获取缓存文件
//                File cacheFile = new File(NewsApplication.getInstance().getCacheDir(), "" + MD5Utils.encode(request.getUrl()));
...
//        }
    ...
    private static void getResult(String result, JsonArrayListener listener) {
            ...
            try {
                ...
                if (resultObj.containsKey("status")) {
                    ...
                    if ("10000".equals(status)) {
                        String jsonResult = resultObj.getString("result");
                        if(!TextUtils.isEmpty(jsonResult) && !"[]".equals(jsonResult)){
                            listener.onJsonArray( 0 , JSON.parseArray(jsonResult));
                        }else{
                            listener.onJsonArrayError();
                        }
                    } else if ("10012".equals(status) || "10014".equals(status)) {
                        NewsApplication.getInstance().sendBroadcast(new Intent("com.babyrun.mmsh.TO_LOGIN"));
//							((IPostCallback) callback).onException();
                        if (resultObj.containsKey("msg") && !"".equals(resultObj.getString("msg"))) {
                            ToastUtil.showNormalShortToast(resultObj.getString("msg"));
                        }
                    } else {
//					((IPostCallback) callback).onException();
                        listener.onJsonArrayError();
...
//		((IPostCallback) callback).onException();
            listener.onJsonArrayError();
//				BroadcastCenter.publish(TITLE.ASYHTTPERR, errMsg);
        }
    }
    private static void getResult(String result, JsonObjectListener listener) {
        if (result != null && !"".equals(result)) {
            try {
                LogManager.i(result.toString());
                JSONObject resultObj = JSON.parseObject(result.toString());
                if (resultObj.containsKey("status")) {
                    ...
                    if ("10000".equals(status)) {
                        String jsonResult = resultObj.getString("result");
                        if(!TextUtils.isEmpty(jsonResult) && !"[]".equals(jsonResult)){
                            listener.onJsonObject(JSON.parseObject(jsonResult));
                        }else{
                            listener.onObjError();
                        }
                    } else if ("10012".equals(status) || "10014".equals(status)) {
...
//					((IPostCallback) callback).onException();
                ...
                } else if (resultObj.containsKey("unionid")) {
                    String jsonResult1 = resultObj.toJSONString();
                    if(!TextUtils.isEmpty(jsonResult1) && !"[]".equals(jsonResult1)){
                        listener.onJsonObject(JSON.parseObject(jsonResult1));
                    }else{
                        listener.onObjError();
                    }
//				((IPostCallback) callback).onAsyHttpSuccess(resultObj.toJSONString());
                } else {
//				((IPostCallback) callback).onException();
                    listener.onObjError();
//						BroadcastCenter.publish(TITLE.ASYHTTPERR, errMsg);
