...
public class TabIndictorView extends LinearLayout {
    ...
    public void setViewPagerAndData(ViewPager viewpager, JSONArray titlesArray, int initTab){
        mTbs.clear();
        for (Object obj : titlesArray) {
            String name = ((JSONObject) obj).getString("title");
            mTbs.add(name);
        }
        mViewPager = viewpager;
        columnSelectIndex=initTab;
